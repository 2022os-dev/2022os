...
impl FileSystems {
    pub fn mount(&mut self, special:
         Special, dir: Path, fstype: FsType) -> isize {
        ...
        for i in 0..len {
            if self.fs_queue[i].1 == dir {
                ...
                for j in 0..self.fs_queue.len() {
                    println!("{} mount to {} , filesystem type is {}", self.fs_queue[j].0, self.fs_queue[j].1, self.fs_queue[j].2);
                }
                return 0
            }
        }
        ...
        for i in 0..self.fs_queue.len() {
            println!("{} mount to {} , filesystem type is {}", self.fs_queue[i].0, self.fs_queue[i].1, self.fs_queue[i].2);
        }
        println!(" ");
