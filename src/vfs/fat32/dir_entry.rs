...
#[allow(unused)]
use super::{
    fat::FAT, fat32_manager::Fat32Manager, get_data_block_buffer, get_info_buffer, BLOCK_SIZE, DEV,
};
...
impl ShortDirEntry {
    ...
    pub fn read_at(
        ...
        let (current, sector_in_cluster, byte_offset) = self.get_offset_position(
            ...
            sectors_per_cluster as u32,
            dev,
            &fat32_manager_read.get_fat(),
        );
        let mut sector_start = byte_offset;
        let mut sector_end: u32 =
            (BLOCK_SIZE as u32).min(byte_offset + need_read_length - have_read_length);
        ...
        'counting_up: loop {
            while current_sector < sectors_per_cluster as u32 {
                let dst = &mut buffer[have_read_length as usize
                    ..(have_read_length + sector_end - sector_start) as usize];
                if self.is_dir() {
                    get_info_buffer(
                        first_sector
    ...
    pub fn get_dir_length(
        ...
        let bytes_per_sector = fat32_manager_read.get_bytes_per_sector();
        sector_per_cluster as u32
            * bytes_per_sector as u32
            * fat.read().get_cluster_num(self.get_start_cluster(), DEV)
    }
    ...
    pub fn increase(
        ...
        let first = fat32_manager_read
            .alloc_cluster(
                ((need_bytes + bytes_per_sector * sectors_per_cluster - 1)
    ...
    pub fn write_at(
        ...
        let mut sector_start = byte_offset;
        let mut sector_end =
            (BLOCK_SIZE as u32).min(byte_offset + total_write_length - have_write_length);
        let mut current_cluster = current;
        let mut current_sector = sector_in_cluster;
        ...
        'counting_up: loop {
            while current_sector < sectors_per_cluster {
                let src = &buffer[have_write_length as usize
                    ..(have_write_length + sector_end - sector_start) as usize];
                if self.is_dir() {
                    get_info_buffer(
    ...
    pub fn trans_to_bytes(&self) -> &[u8] {
        unsafe { core::slice::from_raw_parts(self as *const _ as usize as *const u8, 32) }
    }

    #[allow(unused)]
    pub fn trans_to_mut_bytes(&mut self) -> &mut [u8] {
        unsafe { core::slice::from_raw_parts_mut(self as *mut _ as usize as *mut u8, 32) }
    }
    ...
    pub fn find_next_free_dirent(
        ...
        read_length = self.read_at(
            offset,
            short_dir_entry.trans_to_mut_bytes(),
            dev,
            fat,
            fat32_manager,
        );
        ...
        loop {
            if read_length == 0 {
                ...
                if let Some(cluster) = fat32_manager.write().alloc_cluster(1, dev) {
                    ...
                    // }
                    // else {
                    fat_write.set_next_cluster(
                        fat_write.get_file_last_cluster(self.get_start_cluster(), dev),
                        cluster,
                        dev,
                    );
                    // }
                    let cluster_num =
                        fat_write.get_cluster_num(self.get_start_cluster(), dev) as u32;

                    //返回值，仔细研究
                    return Some(
                        (cluster_num - 1)
                            * fat32_manager_read.get_bytes_per_sector() as u32
                            * fat32_manager_read.get_sectors_per_cluster() as u32,
                    );
                } else {
