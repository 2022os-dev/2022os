...
#[allow(unused)]
use super::{get_data_block_buffer, get_info_buffer, DEV};
...
impl FsInfo {
    ...
    pub fn check_lead_sig(&self, dev: u8) -> bool {
        get_info_buffer(self.sector_number, dev)
            .read()
            .read(0, |&sig: &u32| sig == LEADING_SIGNATURE)
    }

    pub fn check_struct_sig(&self, dev: u8) -> bool {
        get_info_buffer(self.sector_number, dev)
            .read()
            .read(484, |&sig: &u32| sig == STRUCT_SIGNATURE)
    }

    pub fn check_trail_sig(&self, dev: u8) -> bool {
        get_info_buffer(self.sector_number, dev)
            .read()
            .read(508, |&sig: &u32| sig == TRAILING_SIGNATURE)
    }
    ...
    pub fn get_struct_sig(&self, dev: u8) -> u32 {
        get_info_buffer(self.sector_number, dev)
            .read()
            .read(484, |&sig: &u32| sig)
    }

    pub fn get_trail_sig(&self, dev: u8) -> u32 {
        get_info_buffer(self.sector_number, dev)
            .read()
            .read(508, |&sig: &u32| sig)
    }

    pub fn get_free_cluster_num(&self, dev: u8) -> u32 {
        get_info_buffer(self.sector_number, dev)
            .read()
            .read(488, |&num: &u32| num)
    }

    pub fn get_first_free_cluster(&self, dev: u8) -> u32 {
        get_info_buffer(self.sector_number, dev)
            .read()
            .read(492, |&num: &u32| num)
    }

    pub fn set_free_cluster_num(&mut self, free_clustor_num: u32, dev: u8) {
        get_info_buffer(self.sector_number, dev)
            .write()
            .modify(488, |num: &mut u32| *num = free_clustor_num);
    }

    pub fn set_first_free_cluster(&mut self, first_free_clustor: u32, dev: u8) {
        get_info_buffer(self.sector_number, dev)
            .write()
            .modify(492, |num: &mut u32| *num = first_free_clustor);
    }
}
