...
const READ_AND_WRITE: u8 = 0x00000000;
const READ_ONLY: u8 = 0x00000001;
const HIDDEN: u8 = 0x00000010;
const SYSTEM: u8 = 0x00000100;
const LABLE: u8 = 0x00001000;
const SUB_DIRECTORY: u8 = 0x00010000;
const FILING: u8 = 0x00100000;
...
impl ShortDirEntry {
    ...
    pub fn new(file_name: &[u8], extension_name: &[u8], flag: u8) {
    ...
    // 设置文件的起始簇
    pub fn set_start_cluster(&self, start: u32) {
        self.start_cluster_low = start & 0x0000ffff;
        self.start_cluster_high = start >>16;
    }

    // 获取文件起始簇
    pub fn get_start_cluster(&self) -> u32 {
        let mut high: u32 = self.start_cluster_high;
        high = high << 16;
        high + self.start_cluster_low;
    }
    ...
    // 设置文件长度，单位字节
    pub fn set_file_length(&self, u32: file_length) {
    ...
    pub fn get_offset_position(&self, offset: u32 ,sector_per_cluster: u32, dev: u8, fat: &Arc<RwLock<FAT>>,) -> (u32, u32, u32) {
        let cluster_num = offset / (sector_per_cluster * BLOCK_SIZE);
        let cluster_offset = offset % (sector_per_cluster * BLOCK_SIZE);
        let sector_in_cluster: u32 = cluster_offset / BLOCK_SIZE;
        let byte_offset: u32 = cluster_offset % BLOCK_SIZE;
        let mut current: u32 = self.get_start_cluster();
        let fat = fat.read();
        for i in (0..cluster_num) {
            let mut next = fat.get_next_cluster(current ,dev);
            current = next;
        }
    ...
    pub fn read_at(&self, offset: u32, buffer: &mut [u8], dev :u8, fat: &Arc<RwLock<FAT>>, fat32_manager: &Arc<RwLock<Fat32Manager>>) -> u32{
        // 1.前期准备工作
        let fat = fat.read();
        let fat32_manager = fat32_manager.read();
        let bytes_per_sector = fat32_manager.get_bytes_per_sector();
        let sectors_per_cluster = fat32_manager.get_sectors_per_cluster();
        let first_sector = fat32_manager.get_first_sector();
        let total_read_length = buffer.len();
        let mut need_read_length = buffer.len();
        let mut have_read_length = 0;
        let mut file_length: u32;
        // 由于目录项file_length字段置0,所以需手动计算其大小
        if self.is_dir() {
            file_length = fat.get_cluster_num(self.get_start_cluster(), dev) * bytes_per_sector * sectors_per_cluster;
        }
        else {
        ...
        // 2.越界判断,若越界则返回读取到0字节
        if total_read_length + offset - 1 > file_length {
            return 0;
        }

        // 3.找到offset所在的簇号,簇内块号,块内字节偏移,确定块内开始字节与最后读取块内的结束字节,当前读取的簇号,当前读取的簇内块号
        let (current, sector_in_cluster, byte_offset) = get_offset_position(offset,sector_per_cluster, dev, &fat32_manager.get_fat(),);
        let mut sector_start = byte_offset;
        let mut sector_end = BLOCK_SIZE.min(total_read_length - have_read_length);
        ...
        'counting_up: loop: {
            while current_sector < sectors_per_cluster{
                let dst = &mut buf[have_read_length..have_read_length + sector_end - sector_start];
                if self.is_dir() {
                    get_info_cache(
                    ...
                    .read(0, |data_block: &DataBlock| {
                        let src = &data_block[sector_start..sector_end];
                        dst.copy_from_slice(src);
                    });
                    have_read_length += sector_end - sector_start;
                    need_read_length -= sector_end - sector_start;
                    sector_start = 0;
                    sector_end = BLOCK_SIZE.min(total_read_length - have_read_length);
                    current_sector += 1;
            ...
            current_cluster = fat.get_next_cluster(current_cluster, dev);
            current_sector = 0;
    ...
    // 重头戏2,将buffer数组内容写入该文件偏移量为offset处
    pub fn write_at(&self, offset: u32, buffer: & [u8], dev :u8, fat: &Arc<RwLock<FAT>>, fat32_manager: &Arc<RwLock<Fat32Manager>>) {
        // 1.前期准备工作
        let fat = fat.write();
        let fat32_manager = fat32_manager.write();
        let bytes_per_sector = fat32_manager.get_bytes_per_sector();
        let sectors_per_cluster = fat32_manager.get_sectors_per_cluster();
        let first_sector = fat32_manager.get_first_sector();
        let total_write_length = buffer.len();
        let mut need_write_length = buffer.len();
        ...
        if self.is_dir() {
            ...
            file_length = self.get_cluster_num() * bytes_per_sector * sectors_per_cluster;
        }

        // 2.越界判断,若要写的长度加偏移量大于文件长度,则取二者差计算要扩容需要簇的个数,并且调用manager分配
        if total_read_length + offset - 1 > file_length {
            println!("the file do not have so much capacity!!!");
            let need_bytes: usize = total_read_length + offset - file_length;
            let first = fat32_manager.alloc_cluster(need_bytes, dev).unwrap();
            fat.set_next_cluster(fat.get_file_last_cluster(self.get_start_cluster(), dev), first, dev);
        }

        // 3.找到offset所在的簇号,簇内块号,块内字节偏移,确定块内开始字节与最后写入块内的结束字节,当前写入的簇号,当前写入的簇内块号
        let (current, sector_in_cluster, byte_offset) = get_offset_position(offset,sector_per_cluster, dev, &fat32_manager.get_fat(),);
        let mut sector_start = byte_offset;
        let mut sector_end = BLOCK_SIZE.min(total_write_length - have_write_length);
        let mut current_cluster = current;
        let mut current_sector = sector_in_cluster;
        ...
        'counting_up: loop: {
            while current_sector < sectors_per_cluster{
                let src = &buf[have_write_length..have_write_length + sector_end - sector_start];
                if self.is_dir() {
                    get_info_cache(
                        first_sector + current_cluster * sectors_per_cluster + current_sector,
                        dev,
                    ...
                    .write()
                    .modify(0, |data_block: &DataBlock| {
                        let dst = &mut data_block[sector_start..sector_end];
                        dst.copy_from_slice(src);
                    });
                    have_write_length += sector_end - sector_start;
                    need_write_length -= sector_end - sector_start;
                    sector_start = 0;
                    sector_end = BLOCK_SIZE.min(total_write_length - have_write_length);
                    current_sector += 1;
    ...
    pub fn trans_to_mut_bytes(&self) -> &[u8] {
        unsafe {
    ...
    pub fn get_check_sum(&self) -> u8 {
        ...
        for i in (0..8) {
            check_sum = ((check_sum & 1 != 0) ? 0x80 : 0) + (check_sum >> 1) + self.file_name[i];
        }
        for i in (8..11) {
            check_sum = ((check_sum & 1 != 0) ? 0x80 : 0) + (check_sum >> 1) + self.extension_name[i];
        }
        check_sum
    }
    ...
    pub fn find_next_free_dirent(&self, dev: u8, fat: &Arc<RwLock<FAT>>, fat32_manager: &Arc<RwLock<Fat32Manager>>) -> Option(u32) {
        ...
        let mut short_dir_entry = ShortDirEntry::new([0,0,0,0,0,0,0,0], extension_name: [0,0,0], 0);
        let mut read_length = 0;
        read_length = self.read_at(offset, short_dir_entry.as_bytes(), dev, fat, fat32_manager,);
        loop {
            if read_length == 0 {
                ...
                if let Some(cluster) = fat32_manager.write().alloc_cluster(1, dev) {
                    ...
                    let cluster_num = fat_write.get_cluster_num(get_start_cluster(), dev);
    
                    //返回值，仔细研究
                    Some((cluster_num - 1) * fat32_manager_read.get_bytes_per_sector() * fat32_manager_read.get_sectors_per_cluster())
                }
                else{
                    println!("")
                    None
...
impl LongDirEntry {
    ...
    pub fn is_last(&self) -> bool {
        self.flag & 0x01000000 == 1
    }

    pub fn set_last(&self) {
        self.flag |= 0x01000000
    }

    pub fn get_serial(&self) -> u8 {
        self.flag & 0x00011111
    }
